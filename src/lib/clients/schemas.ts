...
// ---------------------------------------------------------------------------
// Base client schema — all optional fields that apply to both particulier/pro
// ---------------------------------------------------------------------------
...
/**
 * Detects potential duplicate clients by email, phone, or SIRET.
 * Returns true if the candidate looks like a duplicate of an existing record.
 *
 * Rules:
 * - email match (case-insensitive, trimmed) → duplicate
 * - phone match (digits only, +33 normalised to 0X) → duplicate
 * - SIRET match → duplicate
 */
export function isDuplicateClient(
  ...
  if (candidate.email && existing.email) {
    if (candidate.email.trim().toLowerCase() === existing.email.trim().toLowerCase()) {
      return true
    }
  }
  ...
  if (candidate.phone && existing.phone) {
    const normalize = (p: string) => {
      ...
      // Normalize French international prefix: +33X... → 0X...
      if (digits.length === 11 && digits.startsWith('33')) {
        return '0' + digits.slice(2)
      }
      return digits
    }
    if (normalize(candidate.phone) === normalize(existing.phone)) {
      return true
    }
  }
