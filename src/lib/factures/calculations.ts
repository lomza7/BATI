...
import type { FactureTotaux } from '@/types/factures'
...
export function calculerTotaux(
  ...
  // Apply discount proportionally to each TVA bucket
  const tvaVentilation = Array.from(tvaMap.entries())
    .sort(([a], [b]) => a - b)
    .map(([rate, baseHtBrut]) => {
      const proportion = totalHtBrut.isZero() ? new Decimal(1) : baseHtBrut.div(totalHtBrut)
  ...
  return {
    ...
    tva_ventilation: tvaVentilation.map(({ rate, base_ht, montant_tva }) => ({
      rate,
      base_ht,
      montant_tva,
    })),
