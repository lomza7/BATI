-- ============================================================
-- Demo seed v3: project photos + enriched team
--
-- Fixes two user-visible demo gaps:
--  1. "Mes chantiers" cards looked empty because no presentation
--     photo was seeded. We now insert 2 photos per project, one
--     with category='presentation' so project cards show a cover.
--  2. "Equipe et sous-traitants" was too thin (2 salaries only).
--     We now seed 3 salaries + 2 sous-traitants (companies with
--     SIRET) so the page shows a realistic mixed team.
--
-- CREATE OR REPLACE requires the full function body; most of
-- the content is identical to 20260410120000_demo_seed_historical.
-- Only the team_members block and the new project_photos block
-- are new. Planning events still reference v_team1 / v_team2.
-- ============================================================
...
DECLARE
  ...
  -- team (3 salaries + 2 sous-traitants)
  v_team1 uuid; v_team2 uuid; v_team3 uuid; v_team4 uuid; v_team5 uuid;
  -- quotes
  ...
  v_m1 timestamptz := date_trunc('month', now()) - interval '3 months';
  v_m2 timestamptz := date_trunc('month', now()) - interval '2 months';
  v_m3 timestamptz := date_trunc('month', now()) - interval '1 month';
  v_m4 timestamptz := date_trunc('month', now());
  v_today date := CURRENT_DATE;
  -- Per-user suffix so quote/invoice numbers are globally unique
  v_suffix text := substr(replace(p_user_id::text, '-', ''), 1, 8);
  v_year text := to_char(now(), 'YYYY');
  -- Photo URLs (confirmed-working Pexels stock photos used elsewhere in the app)
  v_photo_a text := 'https://images.pexels.com/photos/585419/pexels-photo-585419.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750';
  v_photo_b text := 'https://images.pexels.com/photos/2760243/pexels-photo-2760243.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750';
BEGIN
  ...
  -- ============================================================
  -- EQUIPE (3 salaries + 2 sous-traitants)
  -- ============================================================
  INSERT INTO public.team_members (user_id, name, role, type, status, phone, email, specialty, hourly_rate, weekly_hours, start_date, city, color, is_demo, created_at)
  VALUES (p_user_id, 'Karim Belkacem', 'Chef de chantier', 'salarie', 'actif', '06 45 67 89 01', 'karim.b@exemple.fr', 'Maconnerie', 22, 39, (v_m1 - interval '2 years')::date, 'Lyon', '#E97F2A', true, v_m1 - interval '10 days')
  RETURNING id INTO v_team1;

  INSERT INTO public.team_members (user_id, name, role, type, status, phone, email, specialty, hourly_rate, weekly_hours, start_date, city, color, is_demo, created_at)
  VALUES (p_user_id, 'Lucas Moreau', 'Ouvrier polyvalent', 'salarie', 'actif', '06 56 78 90 12', 'lucas.m@exemple.fr', 'Peinture / Platrerie', 16, 35, (v_m1 - interval '14 months')::date, 'Lyon', '#2563eb', true, v_m1 - interval '8 days')
  RETURNING id INTO v_team2;

  INSERT INTO public.team_members (user_id, name, role, type, status, phone, email, specialty, hourly_rate, weekly_hours, start_date, city, color, is_demo, created_at)
  VALUES (p_user_id, 'Hugo Lemoine', 'Apprenti', 'salarie', 'actif', '06 67 89 01 23', 'hugo.l@exemple.fr', 'Carrelage', 12, 35, (v_m2 - interval '4 months')::date, 'Villeurbanne', '#10b981', true, v_m1 - interval '5 days')
  RETURNING id INTO v_team3;

  INSERT INTO public.team_members (user_id, name, role, type, status, phone, email, specialty, hourly_rate, company_name, siret, city, postal_code, color, is_demo, created_at)
  VALUES (p_user_id, 'Jean-Pierre Dubois', 'Gerant', 'sous_traitant', 'actif', '04 78 12 34 56', 'jp.dubois@plomberie-express.fr', 'Plomberie', 35, 'Plomberie Express SARL', '82345678900012', 'Villeurbanne', '69100', '#8b5cf6', true, v_m1 - interval '3 days')
  RETURNING id INTO v_team4;

  INSERT INTO public.team_members (user_id, name, role, type, status, phone, email, specialty, hourly_rate, company_name, siret, city, postal_code, color, is_demo, created_at)
  VALUES (p_user_id, 'Sofiane Amrani', 'Electricien', 'sous_traitant', 'actif', '04 72 88 99 00', 's.amrani@elec-pro.fr', 'Electricite', 40, 'Elec-Pro Services', '91234567800023', 'Venissieux', '69200', '#f59e0b', true, v_m1 - interval '2 days')
  RETURNING id INTO v_team5;
  ...
  -- ============================================================
  -- PHOTOS DES CHANTIERS (2 par projet, 1 "presentation" = cover)
  -- Cascades on project delete so no clear_demo_data change needed.
  -- ============================================================
  INSERT INTO public.project_photos (user_id, project_id, url, caption, category, created_at) VALUES
    (p_user_id, v_p1, v_photo_a, 'Chantier Dupont - vue principale', 'presentation', v_m1 + interval '6 days'),
    (p_user_id, v_p1, v_photo_b, 'Etat avant travaux',                'avant',        v_m1 + interval '4 days'),
    (p_user_id, v_p2, v_photo_b, 'SDB Bernard - rendu final',         'presentation', v_m2 + interval '4 days'),
    (p_user_id, v_p2, v_photo_a, 'Demolition ancienne SDB',           'avant',        v_m1 + interval '16 days'),
    (p_user_id, v_p3, v_photo_a, 'Peinture Martinez - sejour refait', 'presentation', v_m2 + interval '12 days'),
    (p_user_id, v_p3, v_photo_b, 'Pieces avant peinture',             'avant',        v_m1 + interval '25 days'),
    (p_user_id, v_p4, v_photo_b, 'Cuisine Garcia - vue ouverte',      'presentation', v_m2 + interval '25 days'),
    (p_user_id, v_p4, v_photo_a, 'Avant amenagement cuisine',         'avant',        v_m2 + interval '6 days'),
    (p_user_id, v_p5, v_photo_a, 'Veranda Petit - chantier en cours', 'presentation', v_m3 + interval '5 days'),
    (p_user_id, v_p5, v_photo_b, 'Fondations veranda',                'pendant',      v_m2 + interval '18 days'),
    (p_user_id, v_p6, v_photo_b, 'Toiture Roux - refaite',            'presentation', v_m3 + interval '10 days'),
    (p_user_id, v_p6, v_photo_a, 'Ancienne toiture',                  'avant',        v_m2 + interval '23 days'),
    (p_user_id, v_p7, v_photo_a, 'Isolation Moreau - combles',        'presentation', v_m3 + interval '10 days'),
    (p_user_id, v_p7, v_photo_b, 'Combles avant isolation',           'avant',        v_m3 + interval '7 days'),
    (p_user_id, v_p8, v_photo_b, 'Renovation T4 Fabre - salon',       'presentation', v_m4 + interval '1 day'),
    (p_user_id, v_p8, v_photo_a, 'Appartement avant travaux',         'avant',        v_m3 + interval '17 days'),
    (p_user_id, v_p9, v_photo_a, 'Carrelage Simon - pose en cours',   'presentation', v_m4 + interval '2 days'),
    (p_user_id, v_p9, v_photo_b, 'Sol avant pose',                    'avant',        v_m3 + interval '27 days'),
    (p_user_id, v_p10, v_photo_b, 'Bureau Lopez - projet',            'presentation', v_m4 + interval '4 days'),
    (p_user_id, v_p10, v_photo_a, 'Espace a amenager',                'avant',        v_m4 + interval '3 days');
  ...
  INSERT INTO public.planning_events (user_id, title, project_id, team_member_id, event_type, start_date, end_date, all_day, notes, is_demo) VALUES
    ...
    (p_user_id, 'Chantier Bernard SDB - plomberie', v_p2, v_team4, 'chantier', (v_m1 + interval '15 days')::date, (v_m1 + interval '22 days')::date, true, 'Intervention sous-traitant plomberie', true),
    (p_user_id, 'Chantier Bernard SDB - carrelage', v_p2, v_team3, 'chantier', (v_m1 + interval '23 days')::date, (v_m1 + interval '30 days')::date, true, 'Pose carrelage mural et sol', true),
    ...
    (p_user_id, 'Chantier Garcia - electricite', v_p4, v_team5, 'chantier', (v_m2 + interval '12 days')::date, (v_m2 + interval '16 days')::date, true, 'Intervention sous-traitant electricien', true),
    ...
    (p_user_id, 'Chantier Simon carrelage', v_p9, v_team3, 'chantier', (v_m4 + interval '1 day')::date, (v_m4 + interval '12 days')::date, true, 'Pose carrelage', true),
