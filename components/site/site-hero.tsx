...
export function SiteHero({
  ...
  if (effectiveLayout === 'grid') {
    ...
    return (
      <section
        className="relative py-10 sm:py-20 lg:py-24 px-4 sm:px-6"
        style={{
          background: 'linear-gradient(135deg, var(--site-bg) 0%, var(--site-bg-alt) 100%)',
        }}
      >
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10 lg:gap-16 items-center">
          <div className="text-center lg:text-left">
            <h1
              className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-3 sm:mb-6"
              style={{
            ...
            <p
              className="text-sm sm:text-base md:text-lg max-w-xl mb-5 sm:mb-8 mx-auto lg:mx-0"
          ...
          {/* Collage 3 photos */}
          <div className="grid grid-cols-2 grid-rows-2 gap-2 sm:gap-3 h-[220px] sm:h-[380px] lg:h-[500px]">
            <div
  ...
  if (effectiveLayout === 'carousel') {
    ...
    return (
      <section
        className="relative min-h-[60vh] sm:min-h-[70vh] flex items-center justify-center text-center px-4 sm:px-6 overflow-hidden"
        style={{ backgroundColor: 'var(--site-bg-alt)' }}
      >
        <SiteHeroCarousel images={carouselImages} />

        <div className="relative z-10 w-full max-w-3xl mx-auto py-12 sm:py-24">
          <h1
            className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-3 sm:mb-6 text-white"
            style={{ fontFamily: 'var(--site-font)' }}
          >
            {hero.headline}
          </h1>
          <p
            className="text-sm sm:text-base md:text-lg max-w-2xl mx-auto mb-5 sm:mb-8"
  ...
  return (
    <section
      className="relative min-h-[60vh] sm:min-h-[70vh] flex items-center justify-center text-center px-4 sm:px-6"
      style={{ backgroundColor: 'var(--site-bg-alt)' }}
    >
      {singleImage ? (
        <>
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto py-12 sm:py-24">
        <h1
          ...
          style={{
            ...
            fontFamily: 'var(--site-font)',
            textShadow: hasAnyImage ? '0 2px 12px rgba(0,0,0,0.4)' : undefined,
          }}
        >
          {hero.headline}
        </h1>
        ...
        <p
          className="text-sm sm:text-base md:text-lg max-w-2xl mx-auto mb-5 sm:mb-8"
          style={{
            color: hasAnyImage ? 'rgba(255,255,255,0.9)' : 'var(--site-text-muted)',
            textShadow: hasAnyImage ? '0 1px 8px rgba(0,0,0,0.3)' : undefined,
          }}
        >
          {hero.subheadline}
        </p>
        ...
        <p
          className="mt-4 text-xs sm:text-sm"
          style={{ color: hasAnyImage ? 'rgba(255,255,255,0.7)' : 'var(--site-text-muted)' }}
        >
          {companyName} — Devis gratuit et sans engagement
        </p>
      </div>
