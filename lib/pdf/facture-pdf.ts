...
export async function generateFacturePdf(
  ...
  pdfDoc.setTitle(`${facture.invoice_number} — ${profile.company_name ?? profile.full_name ?? ''}`)
  ...
  text(facture.invoice_number, MARGINS.left, PAGE_HEIGHT - 55, { size: 11, color: COLORS.white })
  ...
  if (facture.quote_id) {
    y -= 8
    text(`Réf. devis : ${facture.quote_id}`, MARGINS.left, y, { size: 8, color: COLORS.gray })
    y -= 14
  }
  ...
  page.drawText(facture.invoice_number, {
    x: PAGE_WIDTH / 2 - fontRegular.widthOfTextAtSize(facture.invoice_number, 8) / 2,
    y: 20,
    size: 8,
    font: fontRegular,
    color: COLORS.gray,
